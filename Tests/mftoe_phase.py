...
# ============================================================
# Symmetriereduktion: v → |v|
# ============================================================
def canonical_sol(sol):
    """v → |v| eliminiert Symmetriedopplung v ↔ -v."""
    m, u, v = sol
    return np.array([m, u, abs(v)], dtype=float)


def deduplicate_solutions(solutions, atol=1e-5):
    """Entfernt numerisch äquivalente Lösungen nach Symmetriereduktion."""
    unique = []
    for s in solutions:
        cs = canonical_sol(s)
        if not any(np.allclose(cs, canonical_sol(t), atol=atol, rtol=0.0) for t in unique):
            unique.append(s)
    return unique


def solution_distance(a, b):
    """Gewichtete Distanz im reduzierten Konfigurationsraum."""
    a = canonical_sol(a)
    b = canonical_sol(b)
    dm = 1.2 * abs(a[0] - b[0])
    du = 1.0 * abs(a[1] - b[1])
    ...
    for i in range(len(all_eta)):
        ...
        for j in range(i + 1, len(all_eta)):
            ...
            if abs(all_eta[j] - all_eta[i]) < 2 * (eta_vals[1] - eta_vals[0]) + 1e-10:
                if solution_distance(all_sol[j], all_sol[i]) < 0.12:
                    branch["eta"].append(all_eta[j])
                    branch["sol"].append(all_sol[j])
                    branch["F"].append(all_F[j])
                    used[j] = True
        if len(branch["eta"]) >= 3:  # Filter: mindestens 3 Punkte
            # Sortiere nach eta
            ...
            # Filter: energetisch zu ähnliche Zweige weglassen
            is_duplicate = False
            for prev in branches:
                if (len(branch["eta"]) == len(prev["eta"]) and
                    np.allclose(branch["F"], prev["F"], atol=1e-4)):
                    is_duplicate = True
                    break
            if not is_duplicate:
...
def global_minimum_curve(branches):
    ...
    if not eta_sorted:
        return {k: np.array([]) for k in ["eta","branch_id","m","u","v","F","n_candidates"]}
...
def find_branch_switch_points(global_curve, eps_F=1e-2, eps_m=5e-2, eps_v=5e-2):
    """
    Nur echte Phasenübergänge: Branch-Wechsel + Zustandsunterschied.
    Filtert numerisches Re-Labelling heraus.
    """
    eta  = global_curve["eta"]
    bid  = global_curve["branch_id"]
    m    = global_curve["m"]
    v    = np.abs(global_curve["v"])  # symmetriereduziert
    F    = global_curve["F"]
    ...
    for i in range(1, len(eta)):
        if bid[i] != bid[i - 1]:
            ...
            dm = abs(m[i] - m[i - 1])
            dv = abs(v[i] - v[i - 1])
            # Echter Übergang: energetisch entartet + echter Zustandssprung
            if dF < eps_F and (dm > eps_m or dv > eps_v):
...
def plot_transition_summary_for_lambda(lam, branches):
    if not branches:
        print(f"  Keine Zweige für λ={lam:.2f}, überspringe Plot.")
        return
...
# ============================================================
# Grenzkurve eta_c(lambda) — Extraktion + Fit + Overlay
# ============================================================
def extract_eta_c_from_global_minimum(all_results, lambda_vals_sorted=None, eps_v=1e-3):
    """
    Extrahiert eta_c(lambda) = erster eta, wo |v| <= eps_v im globalen Minimum.
    """
    if lambda_vals_sorted is None:
        lambda_vals_sorted = sorted(all_results.keys())
    ...
    for lam in lambda_vals_sorted:
        ...
        if not branches:
            continue
        gc = global_minimum_curve(branches)
        eta  = np.array(gc["eta"],  dtype=float)
        vabs = np.abs(np.array(gc["v"], dtype=float))
        ...
        aligned = vabs <= eps_v
        if np.any(aligned):
            idx = np.argmax(aligned)
            lambda_pts.append(lam)
            eta_c_pts.append(eta[idx])

    return np.array(lambda_pts, dtype=float), np.array(eta_c_pts, dtype=float)


def fit_eta_c_curve(lambda_pts, eta_c_pts, degree=2):
    """Polynomialer Fit eta_c(lambda) = a0 + a1*lam + a2*lam^2."""
    if len(lambda_pts) < degree + 1:
        raise ValueError(f"Nicht genug Punkte ({len(lambda_pts)}) für Grad {degree}.")
    coeffs = np.polyfit(lambda_pts, eta_c_pts, deg=degree)
    return coeffs, np.poly1d(coeffs)


def plot_eta_c_fit(lambda_pts, eta_c_pts, poly, degree=2):
    """Plot numerischer Grenzpunkte + Fit-Kurve."""
    if len(lambda_pts) == 0:
        print("  Keine Grenzpunkte gefunden, überspringe eta_c-Plot.")
        return

    fig, ax = plt.subplots(figsize=(8, 6))
    ax.scatter(lambda_pts, eta_c_pts, color="black", s=60, zorder=4,
               label=r"numerical $\eta_c(\lambda)$")

    if poly is not None and len(lambda_pts) >= 2:
        lam_dense = np.linspace(lambda_pts.min(), lambda_pts.max(), 400)
        eta_fit   = poly(lam_dense)
        ax.plot(lam_dense, eta_fit, color="royalblue", lw=2.5,
                label=rf"polynomial fit (degree {degree})")

    ax.set_xlabel(r"$\lambda$")
    ax.set_ylabel(r"$\eta_c$")
    ax.set_title(r"Fitted phase boundary $\eta_c(\lambda)$")
    ax.grid(alpha=0.25)
    ax.legend()

    plt.tight_layout()
    fname = os.path.join(SAVE_DIR, "eta_c_fit.png")
    plt.savefig(fname, dpi=180)
    plt.close()
    print(f"  Gespeichert: {fname}")


def overlay_eta_c_on_phase_diagram(all_results, poly=None,
                                   lambda_vals_sorted=None,
                                   eps_v=1e-3, F_degen=1e-3):
    """Phasendiagramm mit überlagerter glatter Fit-Kurve."""
    eta_vals, lambda_vals, phase_map = build_phase_map(
        all_results,
        lambda_vals_sorted=lambda_vals_sorted,
    ...
    im = ax.imshow(
        ...
        extent=[lambda_vals.min(), lambda_vals.max(),
                eta_vals.min(), eta_vals.max()],
        cmap=cmap, vmin=0, vmax=3)

    # Numerische Grenzpunkte
    lpts, epts = extract_eta_c_from_global_minimum(
        all_results, lambda_vals_sorted=lambda_vals, eps_v=eps_v)
    if len(lpts) > 0:
        ax.scatter(lpts, epts, color="white", edgecolor="black",
                   s=60, zorder=4, label=r"numerical $\eta_c$")

    # Glatte Fit-Kurve
    if poly is not None and len(lpts) > 1:
        lam_dense = np.linspace(lambda_vals.min(), lambda_vals.max(), 400)
        ax.plot(lam_dense, poly(lam_dense), color="white", lw=2.5,
                zorder=5, label=r"fit $\eta_c(\lambda)$")

    ax.set_xlabel(r"$\lambda$"); ax.set_ylabel(r"$\eta$")
    ax.set_title(r"Phase diagram with fitted boundary $\eta_c(\lambda)$")
    ...
    fname = os.path.join(SAVE_DIR, "phase_diagram_with_fit.png")
    plt.savefig(fname, dpi=180)
    plt.close()
    print(f"  Gespeichert: {fname}")
...
# ============================================================
# Lokaler Boundary-Scan um eta≈0
# ============================================================
def run_boundary_scan(lambda_cuts_boundary, eta_vals_boundary, n_init=80):
    """
    Feiner lokaler Scan um die Phasengrenze eta_c(lambda).
    Empfohlen: lambda in [0.8, 1.6], eta in [-0.15, 0.15].
    Gibt boundary_results zurück (gleiche Struktur wie all_results).
    """
    boundary_results = {}

    print("  Lokaler Boundary-Scan ...")
    for lam in lambda_cuts_boundary:
        branches = track_branches_for_lambda(lam, eta_vals_boundary, n_init=n_init)
        boundary_results[lam] = branches
        print(f"    λ = {lam:.3f}  →  {len(branches)} Zweige")

    return boundary_results


def plot_boundary_scan_summary(boundary_results, lambda_cuts_boundary,
                                eta_vals_boundary, eps_v=1e-3, degree=2):
    """
    Extrahiert eta_c aus dem feinen Scan und plottet den Fit.
    """
    lambda_pts, eta_c_pts = extract_eta_c_from_global_minimum(
        boundary_results,
        lambda_vals_sorted=sorted(boundary_results.keys()),
        eps_v=eps_v)

    print(f"  Grenzpunkte aus Boundary-Scan ({len(lambda_pts)}):")
    for lam, eta_c in zip(lambda_pts, eta_c_pts):
        print(f"    lambda = {lam:.3f},  eta_c = {eta_c:.6f}")

    poly = None
    if len(lambda_pts) >= degree + 1:
        coeffs, poly = fit_eta_c_curve(lambda_pts, eta_c_pts, degree=degree)
        print(f"\n  Poly-Fit (degree {degree}):")
        terms = ["·".join([f"{c:.4g}", f"lambda^{degree-i}"]) if degree-i > 1
                 else (f"{c:.4g}·lambda" if degree-i == 1 else f"{c:.4g}")
                 for i, c in enumerate(coeffs)]
        print(f"    eta_c(lambda) = {' + '.join(terms)}")

    plot_eta_c_fit(lambda_pts, eta_c_pts, poly, degree=degree)

    # Overlay auf Boundary-Phasendiagramm
    eta_vals_b, lambda_vals_b, phase_map_b = build_phase_map(
        boundary_results,
        lambda_vals_sorted=sorted(boundary_results.keys()),
        eps_v=eps_v)
    ...
    cmap = plt.colormaps.get_cmap("tab10").resampled(4)
    im = ax.imshow(
        phase_map_b, origin="lower", aspect="auto",
        extent=[lambda_vals_b.min(), lambda_vals_b.max(),
                eta_vals_b.min(),   eta_vals_b.max()],
        cmap=cmap, vmin=0, vmax=3)

    if len(lambda_pts) > 0:
        ax.scatter(lambda_pts, eta_c_pts, color="white", edgecolor="black",
                   s=60, zorder=4, label=r"numerical $\eta_c$")

    if poly is not None:
        lam_dense = np.linspace(lambda_vals_b.min(), lambda_vals_b.max(), 400)
        ax.plot(lam_dense, poly(lam_dense), color="white", lw=2.5,
                zorder=5, label=rf"poly fit (deg {degree})")

    ax.set_xlabel(r"$\lambda$"); ax.set_ylabel(r"$\eta$")
    ax.set_title(r"Boundary scan: $\eta_c(\lambda)$ — fine grid")
    cbar = plt.colorbar(im, ax=ax, ticks=[0.5, 1.5, 2.5, 3.5])
    cbar.ax.set_yticklabels(["aligned single", "frustrated single",
                              "aligned multi",  "frustrated multi"])
    ax.legend(loc="best")
    ...
    fname = os.path.join(SAVE_DIR, "boundary_scan_phase_diagram.png")
    plt.savefig(fname, dpi=180)
    plt.close()
    print(f"  Gespeichert: {fname}")

    return lambda_pts, eta_c_pts, poly
...
if __name__ == "__main__":
    ...
    # 4) Grenzkurve eta_c(lambda) fitten
    print("\n--- Schritt 4: Fit der Grenzkurve eta_c(lambda) ---")

    lambda_pts, eta_c_pts = extract_eta_c_from_global_minimum(
        all_results, eps_v=1e-3)

    print("  Numerische Grenzpunkte:")
    for lam, eta_c in zip(lambda_pts, eta_c_pts):
        print(f"    lambda = {lam:.3f},  eta_c = {eta_c:.6f}")
    ...
    if len(lambda_pts) >= 3:
        try:
            coeffs, poly = fit_eta_c_curve(lambda_pts, eta_c_pts, degree=2)
            print(f"\n  Quadratischer Fit:")
            print(f"    eta_c(lambda) = {coeffs[0]:.4g}·lambda^2 "
                  f"+ {coeffs[1]:.4g}·lambda + {coeffs[2]:.4g}")
            plot_eta_c_fit(lambda_pts, eta_c_pts, poly, degree=2)
            overlay_eta_c_on_phase_diagram(
                all_results, poly=poly, eps_v=1e-3, F_degen=1e-3)
        except ValueError as e:
            print(f"  Fit nicht möglich: {e}")
    else:
        print(f"  Nur {len(lambda_pts)} Grenzpunkte — "
              f"mindestens 3 für Fit benötigt.")
        if len(lambda_pts) >= 1:
            plot_eta_c_fit(lambda_pts, eta_c_pts, poly=None, degree=0)
            overlay_eta_c_on_phase_diagram(
                all_results, poly=None, eps_v=1e-3, F_degen=1e-3)

    # 5) Lokaler Boundary-Scan (feines Gitter)
    print("\n--- Schritt 5: Lokaler Boundary-Scan ---")

    lambda_cuts_boundary = np.linspace(0.5, 2.0, 13)    # 13 Punkte
    eta_vals_boundary    = np.linspace(-0.50, 0.10, 121) # fein, eta<0 relevant

    boundary_results = run_boundary_scan(
        lambda_cuts_boundary, eta_vals_boundary, n_init=60)

    plot_boundary_scan_summary(
        boundary_results, lambda_cuts_boundary, eta_vals_boundary,
        eps_v=1e-2, degree=2)
